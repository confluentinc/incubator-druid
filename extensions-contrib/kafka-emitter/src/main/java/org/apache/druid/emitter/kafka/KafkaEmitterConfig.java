...
public class KafkaEmitterConfig
{
  ...
  )
  {
    ...
    this.segmentMetadataTopic = validateSegmentMetadataTopic(segmentMetadataTopic);
    this.clusterName = clusterName;
    this.kafkaProducerConfig = kafkaProducerConfig == null ? ImmutableMap.of() : kafkaProducerConfig;
  }

  private String validateSegmentMetadataTopic(String segmentMetadataTopic) {
    segmentMetadataTopic = this.eventTypes.contains(EventType.SEGMENT_METADATA) ? Preconditions.checkNotNull(segmentMetadataTopic, "segmentMetadata.topic can not be null") : null;
    // Metadata in non-JSON format must go their own topic
    if (segmentMetadataTopic != null && !SegmentMetadataTopicFormat.JSON.equals(this.segmentMetadataTopicFormat)) {
      segmentMetadataTopic += "-" + this.segmentMetadataTopicFormat;
    }
